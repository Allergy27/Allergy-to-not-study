 ...
 * @workspace 25_6_3/8_9_3_MinCostFlow_4016.cpp
 * @date      2025-06-03 15:48:45
 *
 *   ▄████████  ▄█        ▄█          ▄████████    ▄████████    ▄██████▄  ▄██   ▄   
 *  ███    ███ ███       ███         ███    ███   ███    ███   ███    ███ ███   ██▄ 
 *  ███    ███ ███       ███         ███    █▀    ███    ███   ███    █▀  ███▄▄▄███ 
...
signed main(){
    ...
    auto solve = [&]() -> void{
        ...
        MinCostFlow<int> qwq(n + 2);
        int cnt = 0;
        for(auto &x : qvq)std::cin >> x, cnt += x;
        int s = n, t = n + 1;
        int avr = cnt / n;
        for(int i = 0 ; i < n ; ++i){
            qwq.add(i , (i + 1) % n, inf, 1);
            qwq.add((i + 1) % n , i, inf, 1);
            if(qvq[i] > avr){ //out
                qwq.add(s, i, qvq[i] - avr, 0);
            }else{
                qwq.add(i, t, avr - qvq[i], 0);
            }
        }
